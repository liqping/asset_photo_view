...
import 'src/core/photo_view_core.dart';
import 'src/photo_view_computed_scale.dart';
import 'src/photo_view_scale_state.dart';
import 'src/photo_view_wrappers.dart';
import 'src/utils/photo_view_hero_attributes.dart';
...
class _PhotoViewState extends State<PhotoView>
  ...
  void pageDidChanged(){
    ...
    if(_controller.scale != null && _controller.scale != 1){
      final currentPage = widget.pageController?.page;
      if(currentPage != null && widget.key is ObjectKey){
        final myIndex = (widget.key as ObjectKey).value;
        final pageInt = currentPage.toInt();
        if((currentPage - pageInt).abs() < 1e-10){
          if(pageInt != myIndex){
            _controller.reset();
          }
        }
  ...
  void initState() {
    ...
    widget.pageController?.addListener(pageDidChanged);
  ...
  void didUpdateWidget(PhotoView oldWidget) {
    oldWidget.pageController?.removeListener(pageDidChanged);
    widget.pageController?.addListener(pageDidChanged);
    if (widget.controller == null) {
  ...
  void dispose() {
    ...
    widget.pageController?.removeListener(pageDidChanged);
